<%@ Page Language="C#" AutoEventWireup="true" CodeFile="ShowReportDamage.aspx.cs"
    Inherits="WebUI_ReportDamage_ShowReportDamage" %>
...
var userid =<%#UserID %>;
   ...
   {
        id=ID; 
       $.prompt('确定提此次检查吗？<input type ="text" name="alertName" id ="alertName" value="'+ID+'"  style="display:none;" />',{callback:DelOneUser,buttons:{确定:'true',取消:'false'}}); 
       return false; 
   } 
   function DelOneUser(v,m,f)
  {
     ...
     { 
         id =f.alertName;
        var checktxt=$("#textareacheck"+id).val();
   if(checktxt!="")
  {
      $.post("../ashx/VMCheckReportDamage.ashx",{ID:id,UserID:userid,VMDesc:checktxt},function(data){
      if(data.length>0){ 
     $.prompt(data);
      }
      })
  ...
  {
    $.prompt("请输入检查描述信息",{callback:getfocus});
  }
            ...
            <asp:GridView ID="GridView1" runat="server" AutoGenerateColumns="false" Width="899px"
                ...
                <Columns>
                    ...
                    <asp:TemplateField HeaderText="操作">
                        <ItemTemplate>
                            <%#Eval("VMState").ToString()=="1"?"已通过":"<input id=\"Button1\" type=\"button\" value=\"通过\" class =\"qr0\"  onclick=\"return CheckDel("+Eval ("ID")+");\" />"%>
                        </ItemTemplate>
                        <HeaderStyle Width="8%" />
                        <ItemStyle Width="8%" />
                    </asp:TemplateField>
