<%@ Page Language="C#" AutoEventWireup="true" CodeFile="FinalExport.aspx.cs" Inherits="WebUI_Promotion_FinalExport" %>
...
<body>
    ...
    <table class="table">
        ...
        <tr>
            ...
            <td style="text-align: left">
                <span id="checkRackMoney" style=" color:Blue; cursor:pointer; text-decoration:underline;">查 看</span>
            </td>
        </tr>
    </table>
    <br />
    <table class="table">
        <tr>
            <td style="text-align: right; width: 150px;">
                请选择供应商<span style="color:Red;">*</span>:
            </td>
            <td>
        ...
        <tr>
            <td style="text-align: right; width: 150px;">
                请选择大区<span style="color:Red;">*</span>:
            </td>
            <td>
        ...
        <tr>
         ...
         <tr>
            ...
            <td>
                <%--<asp:RadioButtonList ID="rblExportRackMoney" runat="server" 
                    RepeatDirection="Horizontal" RepeatLayout="Flow">
                   <asp:ListItem Value="0" Selected="True">不导出 </asp:ListItem>
                   <asp:ListItem Value="1">导出 </asp:ListItem>
                </asp:RadioButtonList>--%>
                        ...
                        <ItemTemplate>
                           <tr>
                              <td style=" width:80px;border:0px; border-bottom:1px solid #ccc;border-right:1px solid #ccc;">
                                  <%--<asp:CheckBox ID="cbShopLevel" data-val='<%#Eval("ShortName")%>' runat="server"/><%#Eval("ShortName")%>
                              ...
                              <td style=" width:80px;border:0px; border-bottom:1px solid #ccc;border-right:1px solid #ccc;">
                                  <asp:RadioButtonList ID="rblAddressType" runat="server" RepeatLayout="Flow" >
                                    <asp:ListItem Value="1">到店 </asp:ListItem>
    ...
    <div style="text-align: center; width: 900px;">
       <div id="loading" style=" height:30px; text-align:center; display:none;color:Red;">
        ...
        <div id="buttons" style="text-align: center; width: 900px;">
            <asp:Button ID="btnExport" CssClass="qr0" runat="server" Text="导出订单" 
                OnClientClick="return check()" onclick="btnExport_Click"
                />
                &nbsp; &nbsp;
           <asp:Button ID="btnExportRackInfo" runat="server" Text="导出道具" CssClass="qr0" 
                    onclick="btnExportRackInfo_Click" OnClientClick="return ExportRackInfo();"/>
            &nbsp; &nbsp;
            <asp:Button ID="btnBack" runat="server" Text="返 回" CssClass="qr0" onclick="btnBack_Click" />
...
<script type="text/javascript">
    ...
    function check() {
        ...
        var supplierId = $("input:radio[name='rblSupplier']:checked").val() || "";
        var supplierName = $("input:radio[name='rblSupplier']:checked").next().html();
        var regionName = $("input:radio[name='rblRegion']:checked").val() || "";
        ...
        if (canExport) {
            //是否导出道具金额
            //var isExportRackMoney = $("input:radio[name='rblExportRackMoney']:checked").val() || "0";
            $("#buttons").css({ display: "none" });
            $("#loading").css({ display: "" });
            CheckExport();
            var parentStoryBagId = $("#hfParentStoryBagId").val() || "";
            //var sbId = $("#hfSbId").val() || 0;
            var url = "handler/FinalExportOrder.aspx?pid=" + pid + "&sonStoryBagId=" + storyBagId + "&parentStoryBagId=" + parentStoryBagId + "&storyBagName=" + escape(storyBagName) + "&shopLevels=" + escape(shopLevelIds) + "&regionName=" + escape(regionName) + "&supplierId=" + supplierId + "&supplierName=" + escape(supplierName); // + "&isExportRackMoney=" + isExportRackMoney;
    ...
    function ExportRackInfo() {
        ...
        if (canExport) {
            ...
            CheckExport();
            var parentStoryBagId = $("#hfParentStoryBagId").val() || "";
            //var url = "handler/ExportRackDetail.aspx?pid=" + pid + "&sonStoryBagId=" + storyBagId + "&parentStoryBagId=" + parentStoryBagId + "&regionName=" + escape(regionName) + "&supplierId=" + supplierId + "&supplierName=" + escape(supplierName) + "&rackids=" + rackids;
            var url = "handler/ExportRackDetail.aspx?pid=" + pid + "&sonStoryBagId=" + storyBagId + "&parentStoryBagId=" + parentStoryBagId + "&storyBagName=" + escape(storyBagName) + "&shopLevels=" + escape(shopLevelIds) + "&regionName=" + escape(regionName) + "&supplierId=" + supplierId + "&supplierName=" + escape(supplierName); // + "&isExportRackMoney=" + isExportRackMoney;
...
<script type="text/javascript">
    ...
    $(function () {
        ...
        $("#checkRackMoney").click(function () {
            $.fancybox.open({
                href: "RackInfo/FinalOrderRackMoney.aspx?pid="+pid,
                type: "iframe",
                padding: 5,
                width: "90%"
            })
