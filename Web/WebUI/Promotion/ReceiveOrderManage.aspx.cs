...
public partial class WebUI_Promotion_ReceiveOrderManage : System.Web.UI.Page
{
    int userId = 0;
    ...
    protected void Repeater1_ItemDataBound(object sender, RepeaterItemEventArgs e)
    {
        if (e.Item.ItemType == ListItemType.AlternatingItem || e.Item.ItemType == ListItemType.Item)
        {
            ...
            if (item != null)
            {
                ...
                if (list.Any())
                {
                    var list1 = list.Where(s => s.SentOrderDate != null).ToList();
                    ((Label)e.Item.FindControl("labSnedShops")).Text = list1.Count.ToString();

                    var list2 = list.Where(s => s.ReceiveOrderDate != null).ToList();
                    ((Label)e.Item.FindControl("labReceiveShops")).Text = list2.Count.ToString();
                }
            }
        }
    }
