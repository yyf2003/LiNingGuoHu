<%@ Page Language="C#" AutoEventWireup="true" CodeFile="GetShopByCity.aspx.cs" Inherits="WebUI_POPLanuch_GetShopByCity" %>
...
<body onunload="javascript:exit();"  oncontextmenu="self.event.returnValue=false">
    ...
    <div>
        ...
        <tr>
        <th>选择</th><th>店铺名称</th><th>PosCode</th><th>省市</th>
        </tr>
        </HeaderTemplate>
        <ItemTemplate>
        <tr>
        <td>
            <input name="shopcheck" checked="checked" type="checkbox" value='<%#Eval("ShopID") %>' /></td><td><%#Eval("Shopname") %></td><td><%#Eval("PosID") %></td><td><%#Eval("ProvinceName") %>,<%#Eval("cityname") %></td>
        </tr>
        </ItemTemplate>
        <FooterTemplate>
        </table>
